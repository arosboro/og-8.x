...
/**
 * @file
 * Contains \Drupal\og\Plugin\entity_reference\selection\OgSelectionBase.
 */
 ...
 *   id = "og",
 *   label = @Translation("Organic Groups"),
 *   group = "og",
 *   weight = 0
 * )
 */
class OgSelectionBase extends SelectionBase {
  /**
   * {@inheritdoc}
   */
  public static function settingsForm(FieldDefinitionInterface $field_definition) {
    ...
    $bundles = array();
    foreach(entity_get_bundles($target_type) as $bundle_name => $bundle_info) {
    ...
    if (!$bundles) {
      ...
      $form['membership_type'] = array(
        ...
        '#options' => $options,
        '#default_value' => $selection_handler_settings['membership_type'],
        '#required' => TRUE,
      );
    }
  ...
  public function buildEntityQuery($match = NULL, $match_operator = 'CONTAINS') {
    ...
    $group_type = $this->fieldDefinition->getFieldSetting('target_type');
    $entity_info = \Drupal::entityManager()->getDefinition($group_type);

    if (!Field::fieldInfo()->getField($group_type, OG_GROUP_FIELD)) {
      // There are no groups, so falsify query.
      $query->condition($entity_info['entity keys']['id'], -1, '=');
      return $query;
    }
    ...
    // TODO Are we still using field_mode?
    /*
  ...
  private function getGidsForCreate() {
    if ($this->entity->entityType() != 'node') {
      return array();
    }
    ...
    $behavior_settings = $this->fieldDefinition->getFieldSetting('behaviors');
    if (!\Drupal::moduleHandler()->moduleExists('entity_reference_prepopulate') || empty($behavior_settings['prepopulate'])) {
      return array();
    }

    // TODO implement when entityreference_prepopulate is ported to Drupal 8.
    /*
